	...
	If DownloadId	=	DownloadId1 Then
		...
		If StatusCursor.RowCount > 0 Then
			...
			If StatusInt=DownloadManager1.STATUS_FAILED Or StatusInt=DownloadManager1.STATUS_PAUSED Then
				If IsPaused(my_module) = False Then
					CallSubDelayed3(my_module,my_event.ToLowerCase & "_complete",tag_,False)
				End If
			Else
				If IsPaused(my_module) = False Then CallSubDelayed3(my_module,my_event.ToLowerCase & "_complete",tag_,True)
			End If
